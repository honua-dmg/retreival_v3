f = r'averages/NSE/BAJFINANCE/AVGPARSED-2024-12-27.csv'
import pandas as pd
import numpy as np
df = pd.read_csv(f)
...
for x in df.rolling(1):
    ...
    if current_ltp in polariser.keys():
        polariser[current_ltp] +=x['vol-buy'].item()
    else:
        polariser[current_ltp] = x['vol-buy'].item()

    # finding avg quty
    total_buy_qty+=x['vol-buy'].item()
    ...
    # traded time
    traded_time = x['last_traded_time'].values[0]
    ...
    #ax.plot([mean[x]-2*std[x] for x in range(len(mean))],linewidth=1)
    ax.plot(pd.Series(std)+6790,linewidth=1)
    df['vol'] = df['vol-buy'] + df['vol-sell']
    plt.grid()
    plt.title(f)
    plt.pause(0.125)
